    ...
    <motion.div initial="hidden" animate="visible" variants={animate} className={`${row.kickstatus == 'live' ? 'bg-green-100 animate-pulse': row.kickstatus == 'scheduled' ? 'bg-red-50': 'bg-slate-100'} relative overflow-hidden p-2 md:p-4 w-full rounded border flex flex-col md:flex-row items-center justify-start md:justify-center space-y-2 md:space-y-0 md:space-x-5`}>
        ...
        <div className="w-full md:flex-1 flex md:items-center justify-start md:justify-between space-x-7 text-xs">
            ...
            <div className="w-full flex flex-col space-y-1 ">
                <div className="flex items-center justify-between space-x-7">
                    ...
                    <span className={`flex md:hidden ${row.kickstatus == 'scheduled' ? 'font-medium text-gray-600': 'font-black text-red-900'}`}>{['scheduled','postponed'].includes(row.kickstatus) ? '-' : homeGoals }{row.afterPenaltyScore ? <span className="ml-3 font-medium font-mono text-[0.6rem] text-slate-400 italic">P({homeDGoals})</span>:'' }</span>
                </div>
                <div className="flex items-center justify-between space-x-7">
                    <div className="flex items-center space-x-2">
                        <GiRosaShield />
                    ...
                    <span className={`flex md:hidden ${row.kickstatus == 'scheduled' ? 'font-medium text-gray-600': 'font-black text-red-900'}`}>{['scheduled','postponed'].includes(row.kickstatus) ? '-' : awayGoals }{row.afterPenaltyScore ? <span className="ml-3 font-medium font-mono text-[0.6rem] text-slate-400 italic">P({awayDGoals})</span>:'' }</span>
                </div>
            </div>
        </div>
        ...
        <div className="w-fit hidden md:flex justify-end text-xs">
            <div className="flex flex-col space-y-1">
                <div className="flex items-center space-x-2">
                    <span className={`hidden md:flex ${row.kickstatus == 'scheduled' ? 'font-medium text-gray-600': 'font-black text-red-900'}`}>{['scheduled','postponed'].includes(row.kickstatus) ? '-' : homeGoals }{row.afterPenaltyScore ? <span className="ml-2 font-medium font-mono text-[0.6rem] text-slate-400 italic">P({homeDGoals})</span>:''}</span>
                </div>
                <div className="flex items-center space-x-2">
                <span className={`hidden md:flex ${row.kickstatus == 'scheduled' ? 'font-medium text-gray-600': 'font-black text-red-900'}`}>{['scheduled','postponed'].includes(row.kickstatus) ? '-' : awayGoals }{row.afterPenaltyScore ? <span className="ml-2 font-medium font-mono text-[0.6rem] text-slate-400 italic">P({awayDGoals})</span>:''}</span>
                </div>
            </div>
        </div>
    </motion.div>
