...
function GroupPill({ title, data, index }: Props) {
 ...
 <motion.div initial="hidden" animate="visible" variants={animate} className="p-4 bg-white/70 space-y-3 rounded-lg shadow">
    <h1 className="font-bold text-sm text-red-800 tracking-widest">{title}</h1>
    <div className="p-2 w-full rounded-lg bg-white text-[0.65rem] md:text-[0.78rem] font-arial space-y-1">
      <div className="">
      ...
      <div className="font-medium space-y-0.5">
        ...
        <motion.div variants={item} key={row.$id} className={`px-2 py-1 grid grid-cols-8 ${ i == 0 || i == 1 ? 'bg-blue-50' : '' }`}>
          <div className={`col-span-1 ${ i == 0 || i == 1 ? 'text-blue-950 font-black italic' : ''}`}>{i+1}</div>
          <div className="md:mr-2 col-span-4 font-bold text-gray-600 truncate">{row.team}</div>
